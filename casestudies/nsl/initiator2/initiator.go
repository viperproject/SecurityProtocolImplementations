...
pred (a *A) Mem(naT, nbT tm.Term) {
	...
	(a.Version == 2 ==> 
		GetNslLabeling().NonceForEventIsUnique(naT, FinishI)) &&
	(a.Version >= 2 ==>
		acc(lib.Mem(a.na), 1/2) &&
		lib.Abs(a.na) == tm.gamma(naT) &&
...
//@ requires a.Mem(defaultTerm, defaultTerm) && a.Vers(defaultTerm, defaultTerm) == 1
//@ ensures  err == nil ==> a.Mem(naT, defaultTerm) && a.Vers(naT, defaultTerm) == 2
func SendMsg1(a *A /*@, defaultTerm tm.Term @*/) (err error /*@, ghost naT tm.Term @*/) {
	...
	a.Version = 2
	a.na = na
	//@ fold a.Mem(naT, defaultTerm)
}

//@ requires a.Mem(naT, defaultTerm) && a.Vers(naT, defaultTerm) == 2
//@ ensures  err == nil ==> a.Mem(naT, nbT) && a.Vers(naT, nbT) == 3
func RecvMsg2SendMsg3(a *A /*@, ghost naT tm.Term, ghost defaultTerm tm.Term @*/) (err error /*@, ghost nbT tm.Term @*/) {
	//@ unfold a.Mem(naT, defaultTerm)
	...
	if !lib.Compare(a.na, msg2.Na) {
		return lib.NewError("na does not match") /*@, nbT @*/
	}
	if a.IdB != msg2.IdB {
		return lib.NewError("idB does not match") /*@, nbT @*/
	}
	...
	//@ assert labelCtx.IsMsg(s1, msg2T, label.Readers(set[p.Id]{ p.principalId(a.IdA) }))

	/*@
	ghost var msg2Label label.SecrecyLabel
	ghost if labelCtx.IsPublishable(s1, msg2T) {
		msg2Label = label.Public()
	} else {
		msg2Label = label.Readers(set[p.Id]{ p.principalId(a.IdA) })
	}

	labelCtx.IsMsgTupleResolve(s1, msg2T, msg2Label)
	...
	ghost if (!msg2Label.IsPublic()) {
		...
		labelCtx.IsMsgTransitive(s1, nbT, label.Public(), bothLabel)
		labelCtx.PublishableImpliesCorruption(s1, naT, bothLabel)
	}
	@*/

	/*@
	// facts after receiving msg2:
	corruptionOccurred := tr.containsCorruptId(s1.getCorruptIds(), set[p.Id]{ p.principalId(a.IdA), p.principalId(a.IdB) })
	ghost if corruptionOccurred {
		assert labelCtx.IsPublishable(s1, nbT)
	} else {
		assert s1.nonceOccurs(nbT, label.Readers(set[p.Id]{ p.principalId(a.IdA), p.principalId(a.IdB) }), u.Nonce(NslNonce))
		assert s1.eventOccurs(a.IdB, ev.NewEvent(Respond, RespondParams{ a.IdA, a.IdB, naT, nbT }))
	}
	@*/

	/*@
	finishIEv := ev.NewEvent(FinishI, FinishIParams{ a.IdA, a.IdB, naT, nbT })
	fold ctx.eventInv(a.IdA, finishIEv, s1)
	...
	//@ labelCtx.Restrict(s2, nbT, bothLabel, justBLabel)
	//@ labelCtx.IsMsgTupleCreate(s2, msg3T, justBLabel)
	//@ usageCtx.ppredShowWitness(s2, "", msg3T, tm.createPk(a.SkBT), a.IdB, a.IdA, naT)
	...
	a.Version = 3
	// the following assert stmt is needed:
	//@ assert s3.eventOccurs(a.IdA, finishIEv)
	//@ fold a.Mem(naT, nbT)
}
